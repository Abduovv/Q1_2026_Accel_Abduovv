...
use crate::{error::VaultError, TokenVault, VAULT_CONFIG};
...
impl<'info> Initialize<'info> {
    pub fn initialize(
        ...
        self.vault.set_inner(TokenVault {
            administrator: self.admin.key(),
            token_mint: self.mint.key(),
            signer_bump: bump.vault,
        });
        ...
        let base_space =
            ExtensionType::try_calculate_account_len::<Token2022Mint>(&extension_types)
                .map_err(|_| VaultError::InvalidAccountConfig)?;
