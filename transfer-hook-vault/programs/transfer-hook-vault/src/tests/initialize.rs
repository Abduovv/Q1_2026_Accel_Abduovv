...
fn test_initialize() {
    ...
    let vault_data =
        crate::state::TokenVault::try_deserialize(&mut vault_account.data.as_ref()).unwrap();

    let admin_pk = addr_to_pubkey(&admin.pubkey());
    assert_eq!(vault_data.administrator, admin_pk);
    assert_eq!(vault_data.token_mint, mint_pk);
}
